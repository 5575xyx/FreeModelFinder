...
- 全部 5 项配置均改多选（用户已确认）。
- 下拉选项按能力过滤（图片档只列图片模型、视频档只列视频模型、文本档列文本/通用模型）。
- 每档独立轮询游标，互不影响。
- 旧的单字符串配置兼容读取。
...
- Settings 在自动路由区块挂载时加载 `/v1/models` 一次（失败则下拉为空并保留已选显示）。
...
- **server**：连续请求多模型档轮询到不同模型；每档游标独立；旧字符串配置仍可路由；空数组回落。
- **ui**：多选下拉渲染、能力过滤、保存 payload 为数组、空选择清空。
