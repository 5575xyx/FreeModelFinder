...
| GitHub Models | - | 目录中的文本输出模型使用账号自带原型开发额度 | 跳过（缺少密钥） | 若主动启用 paid usage，免费额度后可能计费；非聊天模型已排除 |
...
| Kilo Code | 25 | 白名单 + 零价格双重验证，覆盖 17+ 个免费模型 | 成功 | 免费层约 200 请求/小时；部分模型与 OpenRouter 重叠 |
...
- 跳过：CI 环境缺少 GEMINI_API_KEY 或 GOOGLE_API_KEY。
...
- 跳过：CI 环境缺少 GITHUB_MODELS_TOKEN 或 GITHUB_TOKEN。
