...
> **目录审计于 2026-09-21（Asia/Shanghai）更新：105 个免费模型入口，覆盖 11/13 个 Provider。** 这里统计的是通过免费规则过滤的 Provider 模型入口，同一模型出现在多个 Provider 时会分别计数；本次未发送真实推理请求。
...
与 2026-09-20 相比，成功比较的 11 个 Provider 模型清单没有变化。
